...
import Worker from "./pdftex-worker"

//eslint-disable-next-line
export default function PDFTeX() {
  
  var worker = new WorkerBuilder(Worker);
  var self = this;
  ...
  self.compile = function(source_code) {
    ...
    self.compileRaw(source_code).then(function(binary_pdf) {
      if(binary_pdf === false)
        return p.done(false);
        var pdf_dataurl =  window.btoa(binary_pdf);
  ...
  self.compileRaw = function(source_code) {
    ...
    if(initialized)
      ...
      commands = [
        ...
        curry(self, 'FS_createLazyFilesFromList', ['/', 'http://localhost:5173/texlive.lst' , 'http://localhost:5173/texlive' , true, true]),
      ];
    ...
    var sendCompile = function() {
      ...
      return sendCommand({
        ...
        //'arguments': ['-debug-format', '-output-format', 'pdf', '&latex', 'input.tex',"-s"," ASSERTIONS=1"],
      });
    };
    ...
    var getLog = function() {
      return self.FS_readFile('/input.log');
    }
